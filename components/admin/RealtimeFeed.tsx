...
import { SmartphoneNfc, Store, Clock, Zap } from 'lucide-react';
...
export function useRealtimeFeed(onNewTransaction?: (tx: FeedItem) => void) {
  ...
  // 2. Realtime Listener Subscription (Supabase Realtime or Offline CustomEvent simulation)
  useEffect(() => {
    ...
    const handleNewTransaction = async (payload: any) => {
      ...
      try {
        ...
        const visitor_name = (tag?.visitor as any)?.name || 'Wisatawan';
    ...
    // Handler for offline simulation custom event
    const handleSimulatedTransaction = (e: Event) => {
      const customEvent = e as CustomEvent<Transaction>;
      const enriched = customEvent.detail;

      const newItem: FeedItem = {
        id: enriched.id,
        visitor_name: enriched.visitor_name || 'Wisatawan',
        merchant_name: enriched.merchant_name || 'Merchant Partner',
        merchant_category: enriched.merchant_category || 'General',
        type: enriched.type as 'entry' | 'payment',
    ...
    if (isSupabaseConfigured) {
      ...
      // Offline emulated CustomEvent listener
      window.addEventListener('ecotour_new_transaction', handleSimulatedTransaction);
      return () => {
        window.removeEventListener('ecotour_new_transaction', handleSimulatedTransaction);
      };
    }
  }, [onNewTransaction]);
...
export const RealtimeFeed: React.FC<RealtimeFeedProps> = ({ onNewTransaction }) => {
  ...
  return (
    <div className="w-full lg:w-[320px] bg-white border border-[#e5e3db] rounded-3xl flex flex-col h-[700px] lg:h-[800px] shadow-xs shrink-0 overflow-hidden">
      {/* Header */}
        ...
        <AnimatePresence initial={false}>
          {feedItems.length === 0 ? (
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="h-full flex items-center justify-center text-xs text-gray-400 font-medium py-20"
            >
              Menunggu transaksi...
            </motion.div>
          ) : (
            feedItems.map((item) => (
              <motion.div
                ...
                className={`flex gap-3 p-3 border rounded-2xl text-left transition-all duration-500 ${
                  item.isNew
