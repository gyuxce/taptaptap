import { isSupabaseConfigured, supabase, getStorageItem } from '@/lib/supabase';
...
export async function checkMerchantActivity(): Promise<SilentMerchant[]> {
  ...
  try {
    ...
    if (isSupabaseConfigured) {
      ...
      activeMerchants = merchants.filter(m => m.is_active);

      const txs = getStorageItem<Transaction[]>('ecotour_transactions', []);
      todayTxs = txs.filter(t => t.created_at >= startOfToday.toISOString());
    }
