import { supabase, isSupabaseConfigured, getStorageItem } from '@/lib/supabase';
import { Transaction, Merchant, RFIDTag, Visitor } from '@/types';
...
export async function getRevenueReport(filters: RevenueReportFilters): Promise<RevenueReportItem[]> {
  try {
    ...
    if (isSupabaseConfigured) {
      ...
      allTransactions = (txsData || []).map(tx => {
        const rfidTag = tx.rfid_tag as any;
        return {
          ...tx,
      ...
      // Simulation mode
      merchantsList = getStorageItem<Merchant[]>('ecotour_merchants', []);
      ...
      allTransactions = transactions
        .filter(t => t.created_at >= filters.dateFrom && t.created_at <= filters.dateTo)
        .map(t => {
    ...
    const report: RevenueReportItem[] = merchantsList.map(m => {
      ...
      merchantTxs.forEach(t => {
        if (t.visitor_id) visitorIds.add(t.visitor_id);
      });
...
export async function getDailyRevenue(
  ...
  try {
    ...
    if (isSupabaseConfigured) {
      ...
      allTransactions = transactions
        .filter(t => t.type === 'payment' && t.created_at >= dateFrom && t.created_at <= dateTo)
        .map(t => {
          const m = merchantsList.find(mer => mer.id === t.merchant_id);
          return {
            ...t,
 ...
 */
export async function generateMerchantCommissionReport(
  merchantId: string,
  dateFrom: string,
  dateTo: string,
  ...
  try {
    ...
    if (isSupabaseConfigured) {
      ...
      const transactions = getStorageItem<Transaction[]>('ecotour_transactions', []);
      merchantTxs = transactions.filter(
        t => t.merchant_id === merchantId && t.created_at >= dateFrom && t.created_at <= dateTo
      );
    }
