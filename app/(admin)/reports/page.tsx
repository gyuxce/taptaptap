...
import { Merchant, Transaction, RFIDTag, Visitor } from '@/types';
import { formatRupiah, formatDatetime } from '@/lib/utils';
import { 
  getRevenueReport, 
  getDailyRevenue, 
  generateMerchantCommissionReport,
  RevenueReportItem,
  DailyRevenueItem
...
import { 
  Calendar, Filter, Download, Store, Activity, 
  TrendingUp, FileSpreadsheet, ChevronDown, Check, X, RefreshCw 
} from 'lucide-react';
...
export default function ReportsPage() {
  ...
  }, [dateFrom, dateTo, merchants]);

  const handleApplyFilters = async (isSilent = false) => {
    if (!isSilent) setFiltering(true);
    try {
      ...
      const chart = await getDailyRevenue(
        `${dateFrom}T00:00:00.000Z`,
        `${dateTo}T23:59:59.999Z`,
        selectedMerchantIds.length > 0 ? selectedMerchantIds : undefined
      );
      setChartData(chart);
  ...
  const handleExportCommission = async (merchantId: string, merchantName: string) => {
    try {
      const rep = await generateMerchantCommissionReport(
        merchantId,
        `${dateFrom}T00:00:00.000Z`,
        `${dateTo}T23:59:59.999Z`
      );
  ...
  const handleExportAllTransactions = async () => {
    try {
      ...
      if (isSupabaseConfigured) {
        ...
        const mers = await db.getMerchants();

        txs = allTxs
          .filter(t => t.created_at >= `${dateFrom}T00:00:00.000Z` && t.created_at <= `${dateTo}T23:59:59.999Z`)
          .map(t => {
            const tag = tags.find(tg => tg.uid === t.rfid_uid);
            const visitor = tag ? visitors.find(v => v.id === tag.visitor_id) : null;
            const merchant = mers.find(m => m.id === t.merchant_id);
            return {
              ...t,
        ...
        if (selectedMerchantIds.length > 0) {
          txs = txs.filter(t => selectedMerchantIds.includes(t.merchant_id));
        }
      }
  ...
  return (
    <div className="flex flex-col gap-6 text-left w-full min-w-0">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          <button
            onClick={handleExportMerchantSummary}
            className="flex items-center gap-1.5 text-xs font-bold bg-white border border-[#e5e3db] px-3.5 py-2.5 rounded-xl text-slate-700 cursor-pointer hover:bg-slate-100 transition-all"
          >
          ...
          <div className="flex-1 min-w-[200px]" ref={dropdownRef}>
            ...
            <div className="relative">
              <button
                ...
                className="w-full flex items-center justify-between text-xs font-bold border border-[#e5e3db] px-3.5 py-2 rounded-xl text-slate-700 bg-[#f7f7f5] hover:bg-slate-50 transition-all text-left"
              >
              ...
              {merchantDropdownOpen && (
                <div className="absolute left-0 right-0 mt-1.5 bg-white border border-[#e5e3db] rounded-2xl shadow-xl z-30 max-h-60 overflow-y-auto p-2.5 flex flex-col gap-1">
                  ...
                  <div className="flex justify-between border-b border-slate-100 pb-2 mb-1">
                    ...
                    <button
                      type="button"
                      onClick={clearSelectedMerchants}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard
          ...
          subtext="Akumulasi souvenir digital"
          tone="green"
          icon={<TrendingUp className="h-5.5 w-5.5" />}
        />
        <StatCard
          label="Volume Tapping"
          value={`${summary.totalTaps} Taps`}
          subtext="Taps gerbang & bayar merchant"
          tone="blue"
          icon={<Activity className="h-5.5 w-5.5" />}
        />
        <StatCard
          label="Rata-rata Harian"
          value={formatRupiah(Math.round(summary.avgDailyRevenue))}
          subtext="Total / jumlah hari terfilter"
          tone="blue"
          icon={<Calendar className="h-5.5 w-5.5" />}
        />
        <StatCard
          label="Partner Terlaris"
          value={summary.bestMerchantName}
          subtext={`Omset: ${formatRupiah(summary.bestMerchantRevenue)}`}
          tone="amber"
      ...
      <div className="bg-white border border-[#e5e3db] rounded-2xl p-5 shadow-sm">
        ...
        <div className="h-80 w-full">
          {chartData.length === 0 ? (
            <div className="h-full w-full flex flex-col items-center justify-center border border-dashed border-slate-200 rounded-2xl gap-2 text-slate-400">
            ...
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <Tooltip 
                  ...
                  contentStyle={{ borderRadius: '12px', border: '1px solid #e5e3db', fontSize: '11px', fontWeight: 'bold' }}
                />
                ...
                {chartSeries.length === 0 ? (
                  ...
                  chartSeries.map((key, idx) => (
                    <Bar
                      ...
                      dataKey={key}
                      name={key}
                      stackId="merchant_stack"
                      fill={COLORS[idx % COLORS.length]}
                      radius={[0, 0, 0, 0]} // stacked bars don't need top rounding for middle items
                    />
                  ))
                )}
              </BarChart>
