...
import { Visitor, RFIDTag } from '@/types';
    ...
    return (<div className="flex flex-col gap-6 text-left">
      ...
      <div className="bg-white border border-[#e5e3db] rounded-2xl p-4 flex flex-col md:flex-row gap-3 shadow-xs">
        <div className="flex-grow">
          <Input placeholder="Cari berdasarkan nama, nomor HP, atau UID gelang..." value={searchQuery} onChange={(e) => {
            setSearchQuery(e.target.value);
            setCurrentPage(1);
        }} icon={<Search className="h-4.5 w-4.5"/>}/>
        </div>
        <div className="min-w-[180px]">
          <select value={ticketFilter} onChange={(e) => {
      ...
      <div className="bg-white border border-[#e5e3db] rounded-2xl overflow-hidden shadow-xs">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs border-collapse">
            ...
            return (<React.Fragment key={vis.id}>
                    <tr onClick={() => handleJourneyClick(vis)} className="border-b border-[#f7f7f5] hover:bg-[#f7f7f5]/30 transition-colors cursor-pointer">
                      <td className="p-4">
                        <button onClick={(e) => {
                    e.stopPropagation();
                    handleJourneyClick(vis);
                      ...
                      <td className="py-3 px-2">
                        {vis.credit_limit === 0 ? (<span className="font-extrabold text-[#29ABE2]">Unlimited</span>) : (<div className="flex flex-col gap-1 w-[130px]">
                            <span className="font-bold text-[#1e293b]">
                              {formatRupiah(remaining)}
                            </span>
                            <div className="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden">
                              <div className={`h-full ${progressWidth > 50 ? 'bg-[#29ABE2]' : progressWidth > 20 ? 'bg-amber-500' : 'bg-red-500'}`} style={{ width: `${Math.max(0, Math.min(100, progressWidth))}%` }}/>
                            </div>
                          </div>)}
                      </td>
                    ...
                    e.stopPropagation();
                    handleJourneyClick(vis);
                }} variant="ghost" size="sm" className="text-[10px] font-bold border border-blue-200 text-blue-600 bg-blue-50 hover:bg-blue-100 hover:text-blue-700 rounded-lg cursor-pointer">
                            Journey
                          </Button>
                          <Button onClick={(e) => {
                    e.stopPropagation();
                    handleTopUpClick(vis, tag || null);
                }} disabled={!tag} variant="ghost" size="sm" className="text-[10px] font-bold border border-[#E2EEFF] text-[#29ABE2] bg-[#E8F6FD] hover:bg-[#D5EEFC] hover:text-[#1C95C6] rounded-lg cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed">
                            Top Up
                          </Button>
                          <Button onClick={(e) => {
                    e.stopPropagation();
                    handleEditClick(vis, tag || null);
                }} variant="ghost" size="sm" className="text-[10px] font-bold border border-amber-200 text-amber-600 bg-amber-50 hover:bg-amber-100 hover:text-amber-700 rounded-lg cursor-pointer">
                            Edit
                          </Button>
                          <Button onClick={(e) => {
                    e.stopPropagation();
                    handleResetClick(vis);
                }} disabled={vis.credit_limit === 0 || vis.credit_used === 0} variant="ghost" size="sm" className="text-[10px] font-bold border border-red-200 text-red-500 bg-red-50 hover:bg-red-100 hover:text-red-600 rounded-lg cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed">
                            Reset
                          </Button>
                        </div>
                      </td>
        ...
        {/* Pagination buttons */}
        {totalPages > 1 && (<div className="px-6 py-4 border-t border-[#e5e3db] flex items-center justify-between bg-[#fbfbfa] text-xs">
            ...
            <div className="flex gap-2">
              <Button variant="ghost" size="sm" onClick={() => setCurrentPage(prev => Math.max(1, prev - 1))} disabled={currentPage === 1} className="border border-[#e5e3db] cursor-pointer">
                Sebelumnya
              </Button>
              <Button variant="ghost" size="sm" onClick={() => setCurrentPage(prev => Math.min(totalPages, prev + 1))} disabled={currentPage === totalPages} className="border border-[#e5e3db] cursor-pointer">
                Selanjutnya
              </Button>
            </div>
          </div>)}
      ...
      <EditVisitorModal isOpen={isEditOpen} onClose={() => {
            ...
            setSelectedTag(null);
        }} visitor={selectedVisitor} tag={selectedTag} onSuccess={loadData}/>
      ...
      <VisitorJourneyDrawer isOpen={isJourneyOpen} onClose={() => {
